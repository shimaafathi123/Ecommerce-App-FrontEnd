...
const CartItem = ({ item, increaseQuantity, decreaseQuantity, removeItemFromCart }) => {
    return (
        <tr>
            ...
            <td>
                <button onClick={() => decreaseQuantity(item.id)} disabled={item.quantity === 1} type="button" className="btn btn-outline-secondary btn-sm">
                    <i className="fa-solid fa-minus text-danger"></i>
                </button>
                <span className="mx-2">{item.quantity}</span>
                <button onClick={() => increaseQuantity(item.id)} type="button" className="btn btn-outline-secondary btn-sm">
